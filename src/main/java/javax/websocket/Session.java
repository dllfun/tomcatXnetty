...
public interface Session extends Closeable {
	 ...
	 * {@link MessageHandler} may be registered for each message type (text, binary,
	 * pong). The message type will be derived at runtime from the provided
	 * {@link MessageHandler} instance. It is not always possible to do this so it
	 * is better to use
	 ...
	 * 
	 * @return The current idle timeout for this session in milliseconds. Zero or
	 ...
	 * 
	 * @param timeout The new idle timeout for this session in milliseconds. Zero or
	 *                negative values indicate an infinite timeout.
	 */
	 ...
	 * relied upon to be generated from a secure random source.
	 * 
	 ...
	 * {@link javax.websocket.CloseReason.CloseCodes#NORMAL_CLOSURE} and an empty
	 * reason phrase.
	 *
	...
	/**
	 * Obtain the set of open sessions associated with the same local endpoint as
	 * this session.
	 *
	 * @return The set of currently open sessions for the local endpoint that this
	 *         session is associated with.
	 */
	Set<Session> getOpenSessions();

	/**
	 * Registers a {@link MessageHandler} for partial incoming messages. Only one
	 * {@link MessageHandler} may be registered for each message type (text or
	 * binary, pong messages are never presented as partial messages).
	 *
	...
	/**
	 * Registers a {@link MessageHandler} for whole incoming messages. Only one
	 * {@link MessageHandler} may be registered for each message type (text, binary,
	 * pong).
	 *
	 * @param <T>     The type of message that the given handler is intended for
