 ...
 * NIO2 endpoint.
 */
public class Nio2Endpoint extends SocketWrapperBaseEndpoint<Nio2Channel, AsynchronousSocketChannel> {
	...
	public void stopInternal() {
		...
		getHandler().getProtocol().getExecutor().execute(new Runnable() {
			...
			public void run() {
				...
				try {
					for (Channel<Nio2Channel> wrapper : getConnections()) {
						wrapper.close();
					}
				} catch (Throwable t) {
	...
	protected boolean setSocketOptions(AsynchronousSocketChannel socket) {
		...
		try {
			...
			if (channel == null) {
				...
				if (isSSLEnabled()) {
					channel = new SecureNio2Channel(socketBufferHandler, this);
				} else {
					channel = new Nio2Channel(socketBufferHandler);
				}
			}
	...
	protected class Nio2Acceptor extends Acceptor<AsynchronousSocketChannel>
		...
		public Nio2Acceptor(SocketWrapperBaseEndpoint<?, AsynchronousSocketChannel> endpoint) {
			super(endpoint);
		}
	...
	public static class Nio2SocketWrapper extends SocketWrapperBase<Nio2Channel> {
		...
		private CompletionHandler<Integer, SendfileData> sendfileHandler = new CompletionHandler<Integer, SendfileData>() {
			...
			public void completed(Integer nWrite, SendfileData attachment) {
				...
				if (!buffer.hasRemaining()) {
					if (attachment.length <= 0) {
						...
						if (isInline()) {
							...
							case NONE: {
								getEndpoint().getHandler().processSocket(Nio2SocketWrapper.this, SocketEvent.DISCONNECT,
										false);
								break;
							}
							case PIPELINED: {
				...
				getSocket().write(buffer, Endpoint.toTimeout(getWriteTimeout()), TimeUnit.MILLISECONDS, attachment,
						this);
			}
			...
			public void failed(Throwable exc, SendfileData attachment) {
				...
				if (!isInline()) {
					getEndpoint().getHandler().processSocket(Nio2SocketWrapper.this, SocketEvent.ERROR, false);
				} else {
					attachment.doneInline = true;
		...
		public Nio2SocketWrapper(Nio2Channel channel, final Nio2Endpoint endpoint) {
			...
			this.readCompletionHandler = new CompletionHandler<Integer, ByteBuffer>() {
				...
				public void completed(Integer nBytes, ByteBuffer attachment) {
					...
					if (readNotify) {
						getEndpoint().getHandler().processSocket(Nio2SocketWrapper.this, SocketEvent.OPEN_READ, false);
					}
				}
				...
				public void completed(Integer nBytes, ByteBuffer attachment) {
					...
					synchronized (writeCompletionHandler) {
						if (nBytes.intValue() < 0) {
							...
							// Regular write
							getSocket().write(attachment, Endpoint.toTimeout(getWriteTimeout()), TimeUnit.MILLISECONDS,
		...
		public boolean isReadyForWrite() {
			synchronized (writeCompletionHandler) {
				...
				if (!getWritePending().tryAcquire()) {
					writeInterest = true;
					return false;
				}
				// If the buffer is empty, it is possible to write to it
				if (getSocketBufferHandler().isWriteBufferEmpty() && nonBlockingWriteBuffer.isEmpty()) {
					getWritePending().release();
					return true;
				}
					...
					if (!getSocketBufferHandler().isReadBufferEmpty()) {
						synchronized (readCompletionHandler) {
							...
							for (int i = 0; i < length && !getSocketBufferHandler().isReadBufferEmpty(); i++) {
								nBytes += transfer(getSocketBufferHandler().getReadBuffer(), buffers[offset + i]);
							}
						}
						if (nBytes > 0) {
					...
					if (!getSocketBufferHandler().isWriteBufferEmpty()) {
						synchronized (writeCompletionHandler) {
							...
							ByteBuffer[] array = nonBlockingWriteBuffer
									.toArray(getSocketBufferHandler().getWriteBuffer());
											...
											public void completed(Long nBytes, ByteBuffer[] buffers) {
												if (nBytes.longValue() < 0) {
													...
													getSocket().write(buffers, 0, buffers.length,
															Endpoint.toTimeout(getWriteTimeout()),
		...
		/*
		 * Callers of this method must: - have acquired the readPending semaphore - have
		 * acquired a lock on readCompletionHandler
		 *
		 * This method will release (or arrange for the release of) the readPending
		 ...
		 * {@inheritDoc}
		 * <p>
		 * Overridden for NIO2 to enable a gathering write to be used to write all of
		 * the remaining data in a single additional write should a non-blocking write
		...
		protected void flushBlocking() throws IOException {
			...
			try {
				if (getWritePending().tryAcquire(Endpoint.toTimeout(getWriteTimeout()), TimeUnit.MILLISECONDS)) {
					getWritePending().release();
				} else {
					throw new SocketTimeoutException();
				}
			...
			synchronized (writeCompletionHandler) {
				if (writeNotify || hasPermit || getWritePending().tryAcquire()) {
					...
					if (!nonBlockingWriteBuffer.isEmpty()) {
						ByteBuffer[] array = nonBlockingWriteBuffer.toArray(getSocketBufferHandler().getWriteBuffer());
						Nio2Endpoint.startInline();
						getSocket().write(array, 0, array.length, Endpoint.toTimeout(getWriteTimeout()),
								TimeUnit.MILLISECONDS, array, gatheringWriteCompletionHandler);
						Nio2Endpoint.endInline();
					} else if (getSocketBufferHandler().getWriteBuffer().hasRemaining()) {
						// Regular write
						Nio2Endpoint.startInline();
						getSocket().write(getSocketBufferHandler().getWriteBuffer(),
								Endpoint.toTimeout(getWriteTimeout()), TimeUnit.MILLISECONDS,
								getSocketBufferHandler().getWriteBuffer(), writeCompletionHandler);
						Nio2Endpoint.endInline();
					} else {
						// Nothing was written
						if (!hasPermit) {
		...
		public boolean hasDataToRead() {
			synchronized (readCompletionHandler) {
				return !getSocketBufferHandler().isReadBufferEmpty() || readNotify || getError() != null;
			}
		}
		...
		public boolean isReadPending() {
			synchronized (readCompletionHandler) {
				return getReadPending().availablePermits() == 0;
			}
		}
		...
		public boolean isWritePending() {
			synchronized (writeCompletionHandler) {
				return getWritePending().availablePermits() == 0;
			}
		}
		...
		public boolean awaitReadComplete(long timeout, TimeUnit unit) {
			synchronized (readCompletionHandler) {
				try {
					if (readNotify) {
						return true;
					} else if (getReadPending().tryAcquire(timeout, unit)) {
						getReadPending().release();
						return true;
					} else {
						return false;
		...
		public boolean awaitWriteComplete(long timeout, TimeUnit unit) {
			synchronized (writeCompletionHandler) {
				try {
					if (writeNotify) {
						return true;
					} else if (getWritePending().tryAcquire(timeout, unit)) {
						getWritePending().release();
						return true;
					} else {
						return false;
		...
		public SendfileState processSendfile(SendfileDataBase sendfileData) {
			...
			if (data.fchannel == null || !data.fchannel.isOpen()) {
				...
				try {
					data.fchannel = java.nio.channels.FileChannel.open(path, StandardOpenOption.READ)
							.position(sendfileData.pos);
				} catch (IOException e) {
