...
public class NioBlockingSelector {
	...
	/**
	 * Performs a blocking write using the byte buffer for data to be written If the
	 * <code>selector</code> parameter is null, then it will perform a busy write
	 * that could take up a lot of CPU cycles.
	 *
	 * @param buf          ByteBuffer - the buffer containing the data, we will
	 *                     write as long as <code>(buf.hasRemaining()==true)</code>
		...
		try {
			while (!timedout && buf.hasRemaining()) {
				if (keycount > 0) { // only write if we were registered for a write
					int cnt = socketWrapper.getSocket().write(buf); // write the data
					if (cnt == -1) {
				...
				try {
					...
					att.awaitWriteLatch(Endpoint.toTimeout(writeTimeout), TimeUnit.MILLISECONDS);
	 ...
	 * that could take up a lot of CPU cycles.
	 *
	 * @param buf         ByteBuffer - the buffer containing the data, we will read
	 *                    as until we have read at least one byte or we timed out
	 * @param socket      SocketChannel - the socket to write data to
		...
		try {
			while (!timedout) {
				if (keycount > 0) { // only read if we were registered for a read
					read = socketWrapper.getSocket().read(buf);
					if (read != 0) {
						break;
					}
				}
				try {
					...
					att.awaitReadLatch(Endpoint.toTimeout(readTimeout), TimeUnit.MILLISECONDS);
				} catch (InterruptedException ignore) {
					// Ignore
				}
				if (att.getReadLatch() != null && att.getReadLatch().getCount() > 0) {
	...
	protected static class BlockPoller extends Thread {
		...
		public boolean events() {
			Runnable r = null;
			/*
			 * We only poll and run the runnable events when we start this method. Further
			 * events added to the queue later will be delayed to the next execution of this
			 * method.
			 *
			 * We do in this way, because running event from the events queue may lead the
			 * working thread to add more events to the queue (for example, the worker
			 * thread may add another RunnableAdd event when waken up by a previous
			 * RunnableAdd event who got an invalid SelectionKey). Trying to consume all the
			 * events in an increasing queue till it's empty, will make the loop hard to be
			 * terminated, which will kill a lot of time, and greatly affect performance of
			 * the poller loop.
			 */
