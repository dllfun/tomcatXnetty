...
public class NioSelectorPool {
	...
	protected boolean shared = Boolean
			.parseBoolean(System.getProperty("org.apache.tomcat.util.net.NioSelectorShared", "true"));
	...
	/**
	 * Performs a write using the bytebuffer for data to be written and a selector
	 * to block (if blocking is requested). If the <code>selector</code> parameter
	 * is null, and blocking is requested then it will perform a busy write that
	 * could take up a lot of CPU cycles.
	 * 
	 * @param buf          The buffer containing the data, we will write as long as
	 *                     <code>(buf.hasRemaining()==true)</code>
	 * @param socket       The socket to write data to
	 * @param selector     The selector to use for blocking, if null then a busy
	 *                     write will be initiated
	 * @param writeTimeout The timeout for this write operation in milliseconds, -1
	 *                     means no timeout
		...
		try {
			while ((!timedout) && buf.hasRemaining()) {
				...
				if (keycount > 0) { // only write if we were registered for a write
					cnt = socketWrapper.getSocket().write(buf); // write the data
					if (cnt == -1) {
				...
				if (selector != null) {
					...
					if (key == null) {
						key = socketWrapper.getSocket().getIOChannel().register(selector, SelectionKey.OP_WRITE);
					} else {
	 ...
	 * selector to block. If the <code>selector</code> parameter is null, then it
	 * will perform a busy read that could take up a lot of CPU cycles.
	 * 
	 ...
	 * @param selector    Selector - the selector to use for blocking, if null then
	 *                    a busy read will be initiated
		...
		try {
			while (!timedout) {
				...
				if (keycount > 0) { // only read if we were registered for a read
					cnt = socketWrapper.getSocket().read(buf);
					if (cnt == -1) {
						if (read == 0) {
							read = -1;
						}
				...
				if (selector != null) {// perform a blocking read
					// register OP_WRITE to the selector
					if (key == null) {
						key = socketWrapper.getSocket().getIOChannel().register(selector, SelectionKey.OP_READ);
					} else
