...
public class StandardJarScanner implements JarScanner {
	...
	/**
	 * Controls the testing of the bootstrap classpath which consists of the runtime
	 * classes provided by the JVM and any installed system extensions.
	 */
	...
	/*
	 * Since class loader hierarchies can get complicated, this method attempts to
	 * apply the following rule: A class loader is a web application class loader
	 * unless it loaded this class (StandardJarScanner) or is a parent of the class
	 * loader that loaded this class.
	 *
	 * This should mean: the webapp class loader is an application class loader the
	 * shared class loader is an application class loader the server class loader is
	 * not an application class loader the common class loader is not an application
	 * class loader the system class loader is not an application class loader the
	 * bootstrap class loader is not an application class loader
	 */
	...
	/*
	 * Scan a URL for JARs with the optional extensions to look at all files and all
	 * directories.
	 */
		...
		if (manifest != null) {
			...
			for (String classPathEntry : classPathEntries) {
				...
				try {
					...
					/*
					 * Note: Resolving the relative URLs from the manifest has the potential to
					 * introduce security concerns. However, since only JARs provided by the
					 * container and NOT those provided by web applications are processed, there
					 * should be no issues. If this feature is ever extended to include JARs
					 * provided by web applications, checks should be added to ensure that any
					 * relative URL does not step outside the web application.
					 */
