...
public final class ByteChunk extends AbstractChunk {
	...
	public static interface ByteInputChannel {
		...
		// public BufWrapper getBufWrapper();
	}

	/**
	 * When we need more space we'll either grow the buffer ( up to the limit ) or
	...
	public static interface ByteOutputChannel {
		...
		/**
		 * Send the bytes ( usually the internal conversion buffer ). Expect 8k output
	...
	/**
	 * When the buffer is full, write the data to the output channel. Also used when
	 * large amount of data is appended. If not set, the buffer will grow to the
	 * limit.
	 *
	...
	/*
	 * @deprecated Use {@link #subtractB()}. This method will be removed in Tomcat
	 * 10
	 */
	@Deprecated
	 ...
	 *
	 * @deprecated Use {@link #subtract(ByteBuffer)}. This method will be removed in
	 *             Tomcat 10
	 */
	@Deprecated
	...
	private boolean checkEof() throws IOException {
		if ((end - start) == 0) {
			...
			int n = -1;// in.realReadBytes();
			BufWrapper bufWrapper = in.realReadBytes();
			if (bufWrapper != null) {
	...
	/**
	 * Returns true if the buffer starts with the specified string when tested in a
	...
	/**
	 * Returns the first instance of the given character in this ByteChunk starting
	 * at the specified byte. If the character is not found, -1 is returned. <br>
	...
	/**
	 * Returns the first instance of the given byte in the byte array between the
	 ...
	 * @return The position of the first instance of the byte or -1 if the byte is
	 *         not found.
	 */
