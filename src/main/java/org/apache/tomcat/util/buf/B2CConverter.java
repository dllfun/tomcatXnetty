...
public class B2CConverter {
	...
	public void convert(BufWrapper bc, CharBuffer cc, ByteChunk.ByteInputChannel ic, boolean endOfInput)
		...
		ByteBuffer bb = null;
		if (bc.hasArray()) {
			bb = ByteBuffer.wrap(bc.getArray(), bc.getPosition(), bc.getRemaining());// bc.arrayOffset() +
		} else {
			bb = bc.nioBuffer();
		}
		// if ((bb == null) || (bc.hasArray() && bb.array() != bc.getArray())) {
		// Create a new byte buffer if anything changed
		// bb = ByteBuffer.wrap(bc.array(), bc.arrayOffset() + bc.position(),
		// bc.remaining());
		// } else {
		// Initialize the byte buffer
		// bb.limit(bc.limit());
		...
		if (leftovers.position() > 0) {
			...
			do {
				...
				if (bc.getRemaining() == 0) {
					int n = -1;// ic.realReadBytes();
					bc = ic.realReadBytes();
					if (bc != null) {
						n = bc.getRemaining();
					}
					// bc = ic.getBufWrapper();
					chr = n < 0 ? -1 : bc.getByte();
				} else {
					chr = bc.getByte();
				}
				leftovers.put(chr);
		...
		if (result.isError() || result.isMalformed()) {
			...
			// Propagate current positions to the byte chunk and char chunk, if
			// this continues the char buffer will get resized
			bc.setPosition(bc.getPosition() + bb.position() - pos);
