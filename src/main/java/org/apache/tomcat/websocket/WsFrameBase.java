...
public abstract class WsFrameBase {
	...
	/**
	 * @return <code>true</code> if sufficient data was present to process all of
	 *         the initial header
	 */
	...
	/**
	 * WAITING - not suspended Server case: waiting for a notification that data is
	 * ready to be read from the socket, the socket is registered to the poller
	 * Client case: data has been read from the socket and is waiting for data to be
	 * processed PROCESSING - not suspended Server case: reading from the socket and
	 * processing the data Client case: processing the data if such has already been
	 * read and more data will be read from the socket SUSPENDING_WAIT - suspended,
	 * a call to suspend() was made while in WAITING state. A call to resume() will
	 * do nothing and will transition to WAITING state SUSPENDING_PROCESS -
	 * suspended, a call to suspend() was made while in PROCESSING state. A call to
	 * resume() will do nothing and will transition to PROCESSING state SUSPENDED -
	 * suspended Server case: processing data finished (SUSPENDING_PROCESS) / a
	 * notification was received that data is ready to be read from the socket
	 * (SUSPENDING_WAIT), socket is not registered to the poller Client case:
	 * processing data finished (SUSPENDING_PROCESS) / data has been read from the
	 * socket and is available for processing (SUSPENDING_WAIT) A call to resume()
	 * will: Server case: register the socket to the poller Client case: resume data
	 * processing CLOSING - not suspended, a close will be send
	 *
	 * <pre>
	...
	protected enum ReadState {
		WAITING(false), PROCESSING(false), SUSPENDING_WAIT(true), SUSPENDING_PROCESS(true), SUSPENDED(true),
		CLOSING(false);
...
//	}
	...
	/**
	 * This method will be invoked when the read operation is resumed. As the
	 * suspend of the read operation can be invoked at any time, when implementing
	 * this method one should consider that there might still be data remaining into
	 * the internal buffers that needs to be processed before reading again from the
	 * socket.
	 */
	...
	/**
	 * For use by the server implementation that needs to obtain payload data and
	 * unmask it before any further processing.
	 */
