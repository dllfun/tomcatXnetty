...
public class PerMessageDeflate implements Transformation {
	...
	private final ByteBufferWrapper readBuffer = ByteBufferWrapper
	...
	private volatile ByteBufferWrapper writeBuffer = ByteBufferWrapper
			.wrapper(ByteBuffer.allocate(Constants.DEFAULT_BUFFER_SIZE), false);
	...
	public TransformationResult getMoreData(byte opCode, boolean fin, int rsv, ByteBufferWrapper dest)
		...
		while (dest.getRemaining() > 0) {
			// Space available in destination. Try and fill it.
			try {
				written = inflater.inflate(dest.getArray(), dest.getArrayOffset() + dest.getPosition(),
						dest.getRemaining());
			...
			dest.setPosition(dest.getPosition() + written);

			if (inflater.needsInput() && !usedEomBytes) {
				if (dest.hasRemaining()) {
					readBuffer.switchToWriteMode();
					readBuffer.clearWrite();
					TransformationResult nextResult = next.getMoreData(opCode, fin, (rsv ^ RSV_BITMASK), readBuffer);
					inflater.setInput(readBuffer.getArray(), readBuffer.getArrayOffset(), readBuffer.getPosition());
					if (TransformationResult.UNDERFLOW.equals(nextResult)) {
						return nextResult;
					} else if (TransformationResult.END_OF_FRAME.equals(nextResult) && readBuffer.getPosition() == 0) {
						if (fin) {
	...
	public List<MessagePart> sendMessagePart(List<MessagePart> uncompressedParts) throws IOException {
		...
		for (MessagePart uncompressedPart : uncompressedParts) {
			...
			boolean emptyPart = uncompressedPart.getPayload().getLimit() == 0;
			...
			if (Util.isControl(opCode)) {
				...
				ByteBufferWrapper uncompressedPayload = uncompressedPart.getPayload();
				SendHandler uncompressedIntermediateHandler = uncompressedPart.getIntermediateHandler();

				deflater.setInput(uncompressedPayload.getArray(),
						uncompressedPayload.getArrayOffset() + uncompressedPayload.getPosition(),
						uncompressedPayload.getRemaining());

				int flush = (uncompressedPart.isFin() ? Deflater.SYNC_FLUSH : Deflater.NO_FLUSH);
				boolean deflateRequired = true;

				while (deflateRequired) {
					ByteBufferWrapper compressedPayload = writeBuffer;

					try {
						int written = deflater.deflate(compressedPayload.getArray(),
								compressedPayload.getArrayOffset() + compressedPayload.getPosition(),
								compressedPayload.getRemaining(), flush);
						compressedPayload.setPosition(compressedPayload.getPosition() + written);
					} catch (NullPointerException e) {
						throw new IOException(sm.getString("perMessageDeflate.alreadyClosed"), e);
					}
					...
					writeBuffer = ByteBufferWrapper.wrapper(ByteBuffer.allocate(Constants.DEFAULT_BUFFER_SIZE), false);

					// Flip the compressed payload ready for writing
//					compressedPayload.flip();
					compressedPayload.switchToReadMode();

					boolean fin = uncompressedPart.isFin();
					boolean full = compressedPayload.getLimit() == compressedPayload.getCapacity();
					...
					if (fin && !full && needsInput) {
						...
						compressedPayload.setLimit(compressedPayload.getLimit() - EOM_BYTES.length);
						...
						if (eomBufferWritten < EOM_BUFFER.length) {
							...
							compressedPayload
									.setLimit(compressedPayload.getLimit() - EOM_BYTES.length + eomBufferWritten);
							compressedPart = new MessagePart(true, getRsv(uncompressedPart), opCode, compressedPayload,
									uncompressedIntermediateHandler, uncompressedIntermediateHandler,
									blockingWriteTimeoutExpiry);
							deflateRequired = false;
							...
							writeBuffer.putBytes(EOM_BUFFER, 0, eomBufferWritten);
							compressedPart = new MessagePart(false, getRsv(uncompressedPart), opCode, compressedPayload,
									uncompressedIntermediateHandler, uncompressedIntermediateHandler,
									blockingWriteTimeoutExpiry);
						}
