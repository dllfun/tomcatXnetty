...
public interface Transformation {
	...
	/**
	 * Validate that the RSV bit(s) required by this transformation are not being
	 * used by another extension. The implementation is expected to set any bits it
	 * requires before passing the set of in-use bits to the next transformation.
	 *
	 * @param i The RSV bits marked as in use so far as an int in the range zero to
	 ...
	 *                     transform
	 */
	TransformationResult getMoreData(byte opCode, boolean fin, int rsv, ByteBufferWrapper dest) throws IOException;

	/**
	 * Validates the RSV and opcode combination (assumed to have been extracted from
	 * a WebSocket Frame) for this extension. The implementation is expected to
	 * unset any RSV bits it has validated before passing the remaining RSV bits to
	 * the next transformation in the pipeline.
	 *
	 * @param rsv    The RSV bits received as an int in the range zero to seven with
	 *               RSV1 as the MSB and RSV3 as the LSB
	...
	/**
	 * Takes the provided list of messages, transforms them, passes the transformed
	 * list on to the next transformation (if any) and then returns the resulting
	 * list of message parts after all of the transformations have been applied.
	 *
	 * @param messageParts The list of messages to be transformed
	 *
	 * @return The list of messages after this any any subsequent transformations
	 *         have been applied. The size of the returned list may be bigger or
	 *         smaller than the size of the input list
	 *
