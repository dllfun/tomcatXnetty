...
public class WsWebSocketContainer implements WebSocketContainer, BackgroundProcess {
	...
	public Session connectToServer(Object pojo, URI path) throws DeploymentException {
		...
		if (annotation == null) {
			throw new DeploymentException(
					sm.getString("wsWebSocketContainer.missingAnnotation", pojo.getClass().getName()));
		}
	...
	public Session connectToServer(Class<?> annotatedEndpointClass, URI path) throws DeploymentException {
		...
		try {
			...
			throw new DeploymentException(
					sm.getString("wsWebSocketContainer.endpointCreateFail", annotatedEndpointClass.getName()), e);
		}
	...
	private Session connectToServerRecursive(Endpoint endpoint, ClientEndpointConfig clientEndpointConfiguration,
			URI path, Set<URI> redirectSet) throws DeploymentException {

		boolean secure = false;
		ByteBufferWrapper proxyConnect = null;
		URI proxyPath;
		...
		ByteBufferWrapper request = createRequest(path, reqHeaders);
		...
		ByteBufferWrapper response = ByteBufferWrapper
				.wrapper(ByteBuffer.allocate(getDefaultMaxBinaryMessageBufferSize()), false);
		String subProtocol;
		...
		try {
			...
			if (httpResponse.status != 101) {
				if (isRedirectStatus(httpResponse.status)) {
					...
					String requestUri = new String(request.getArray(), StandardCharsets.ISO_8859_1).split("\\s", 3)[1];
		...
		WsSession wsSession = new WsSession(endpoint, wsRemoteEndpointClient, this, null, null, null, null, null,
				extensionsAgreed, subProtocol, Collections.<String, String>emptyMap(), secure,
	...
	private static void writeRequest(AsyncChannelWrapper channel, ByteBufferWrapper request, long timeout)
			throws TimeoutException, InterruptedException, ExecutionException {
		int toWrite = request.getLimit();
	...
	private static ByteBufferWrapper createProxyRequest(String host, int port) {
		...
		return ByteBufferWrapper.wrapper(ByteBuffer.wrap(bytes), true);
	}
	...
	private static ByteBufferWrapper createRequest(URI uri, Map<String, List<String>> reqHeaders) {
		ByteBufferWrapper result = ByteBufferWrapper.wrapper(ByteBuffer.allocate(4 * 1024), false);

		// Request line
		result.putBytes(GET_BYTES);
		if (null == uri.getPath() || "".equals(uri.getPath())) {
			result.putBytes(ROOT_URI_BYTES);
		} else {
			result.putBytes(uri.getRawPath().getBytes(StandardCharsets.ISO_8859_1));
		}
		String query = uri.getRawQuery();
		if (query != null) {
			result.putByte((byte) '?');
			result.putBytes(query.getBytes(StandardCharsets.ISO_8859_1));
		}
		result.putBytes(HTTP_VERSION_BYTES);

		// Headers
		for (Entry<String, List<String>> entry : reqHeaders.entrySet()) {
			addHeader(result, entry.getKey(), entry.getValue());// result =
		}

		// Terminating CRLF
		result.putBytes(CRLF);
...
//		result.flip();
	...
	private static void addHeader(ByteBufferWrapper result, String key, List<String> values) {
		if (values.isEmpty()) {
			return;// result;
		}

		putWithExpand(result, key.getBytes(StandardCharsets.ISO_8859_1));
		putWithExpand(result, ": ".getBytes(StandardCharsets.ISO_8859_1));
		putWithExpand(result, StringUtils.join(values).getBytes(StandardCharsets.ISO_8859_1));
		putWithExpand(result, CRLF);

//		return result;
	}

	private static void putWithExpand(ByteBufferWrapper input, byte[] bytes) {
		if (bytes.length > input.getRemaining()) {
			int newSize;
			if (bytes.length > input.getCapacity()) {
				newSize = 2 * bytes.length;
			} else {
				newSize = input.getCapacity() * 2;
			}
			ByteBuffer expanded = ByteBuffer.allocate(newSize);
			input.expand(newSize);
//			input.flip();
//			expanded.put(input);
//			input = expanded;
		}
		input.putBytes(bytes);
//		return input;
	}
	 ...
	 */
	private HttpResponse processResponse(ByteBufferWrapper response, AsyncChannelWrapper channel, long timeout)
		...
		while (!readHeaders) {
			...
			response.switchToWriteMode();
			response.getByteBuffer().clear();
			// Blocking read
			Future<Integer> read = channel.read(response.getByteBuffer());
			Integer bytesRead = read.get(timeout, TimeUnit.MILLISECONDS);
			if (bytesRead.intValue() == -1) {
				throw new EOFException();
			}
//			response.flip();
			response.switchToReadMode();
	...
	private String readLine(ByteBufferWrapper response) {
		// All ISO-8859-1
		StringBuilder sb = new StringBuilder();

		char c = 0;
		while (response.hasRemaining()) {
			c = (char) response.getByte();
			sb.append(c);
			if (c == 10) {
				break;
			}
	...
	/**
	 * Cleans up the resources still in use by WebSocket sessions created from this
	 * container. This includes closing sessions and cancelling {@link Future}s
	 * associated with blocking read/writes.
	 */
	public void destroy() {
	 ...
	 * {@inheritDoc}
	 *
	 * The default value is 10 which means session expirations are processed every
	 * 10 seconds.
	 */
	@Override
