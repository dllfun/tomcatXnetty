...
public class WsSession implements Session {
	...
	/**
	 * Creates a new WebSocket session for communication between the two provided
	 * end points. The result of {@link Thread#getContextClassLoader()} at the time
	 * this constructor is called will be used when calling
	 ...
	 * @param requestParameterMap  The parameters associated with the request that
	 *                             initiated this session or <code>null</code> if
	 *                             this is a client session
	 * @param queryString          The query string associated with the request that
	 *                             initiated this session or <code>null</code> if
	 *                             this is a client session
	 * @param userPrincipal        The principal associated with the request that
	 *                             initiated this session or <code>null</code> if
	 *                             this is a client session
	 * @param httpSessionId        The HTTP session ID associated with the request
	 *                             that initiated this session or <code>null</code>
	 *                             if this is a client session
	 ...
	 * @param pathParameters       The path parameters associated with the request
	 *                             that initiated this session or <code>null</code>
	 *                             if this is a client session
	 ...
	 * @param closeReasonMessage The close reason to pass to the remote endpoint
	 * @param closeReasonLocal   The close reason to pass to the local endpoint
	 * @param closeSocket        Should the socket be closed immediately rather than
	...
	/**
	 * Called when a close message is received. Should only ever happen once. Also
	 * called after a protocol error when the ProtocolHandler needs to force the
...
//		msg.flip();
	...
	/**
	 * Make the session aware of a {@link FutureToSendHandler} that will need to be
	 * forcibly closed if the session closes before the {@link FutureToSendHandler}
	 * completes.
	 * 
	...
	private void checkState() {
		if (state == State.CLOSED) {
			/*
			 * As per RFC 6455, a WebSocket connection is considered to be closed once a
			 * peer has sent and received a WebSocket close frame.
			 */
