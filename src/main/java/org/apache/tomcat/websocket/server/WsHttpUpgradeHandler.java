...
public class WsHttpUpgradeHandler implements InternalHttpUpgradeHandler {
	...
	public void preInit(Endpoint endpoint, ServerEndpointConfig serverEndpointConfig, WsServerContainer wsc,
	...
	public void init(WebConnection connection) {
		...
		try {
			wsRemoteEndpointServer = new WsRemoteEndpointImplServer(channel, webSocketContainer);
			wsSession = new WsSession(endpoint, wsRemoteEndpointServer, webSocketContainer,
					handshakeRequest.getRequestURI(), handshakeRequest.getParameterMap(),
					handshakeRequest.getQueryString(), handshakeRequest.getUserPrincipal(), httpSessionId,
					negotiatedExtensions, subProtocol, pathParameters, secure, serverEndpointConfig);
			wsFrame = new WsFrameServer(channel, wsSession, transformation, applicationClassLoader);
	...
	private void close(CloseReason cr) {
		/*
		 * Any call to this method is a result of a problem reading from the client. At
		 * this point that state of the connection is unknown. Attempt to send a close
		 * frame to the client and then close the socket immediately. There is no point
		 * in waiting for a close frame from the client because there is no guarantee
		 * that we can recover from whatever messed up state the client put the
		 * connection into.
		 */
