...
public class AsyncChannelWrapperSecure implements AsyncChannelWrapper {
	...
	public Future<Integer> write(ByteBufferWrapper src) {
		...
		WriteTask writeTask = new WriteTask(new ByteBufferWrapper[] { src }, 0, 1, inner);
	...
	@Override
	public <B, A extends B> void write(ByteBufferWrapper[] srcs, int offset, int length, long timeout, TimeUnit unit,
	...
	private class WriteTask implements Runnable {
		...
		private final ByteBufferWrapper[] srcs;
		private final int offset;
		private final int length;
		private final WrapperFuture<Long, ?> future;

		public WriteTask(ByteBufferWrapper[] srcs, int offset, int length, WrapperFuture<Long, ?> future) {
			this.srcs = srcs;
			this.future = future;
			this.offset = offset;
			this.length = length;
			...
			try {
				for (int i = offset; i < offset + length; i++) {
					...
					while (src.hasRemaining()) {
						...
						// Encrypt the data
						SSLEngineResult r = sslEngine.wrap(src.getByteBuffer(), socketWriteBuffer);
