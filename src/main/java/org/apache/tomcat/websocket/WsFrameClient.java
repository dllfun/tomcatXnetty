...
public class WsFrameClient extends WsFrameBase {
	...
	private volatile ByteBufferWrapper response;

	public WsFrameClient(ByteBufferWrapper response, AsyncChannelWrapper channel, WsSession wsSession,
...
//					inputBuffer.setLimit(inputBuffer.getCapacity());
					inputBuffer.switchToWriteMode(false);

					// Copy remaining bytes read in HTTP phase to input buffer used by
					// frame processing

//					int orgLimit = response.getLimit();
//					int toCopy = Math.min(response.getRemaining(), inputBuffer.getRemaining());
//					response.setLimit(response.getPosition() + toCopy);
//					inputBuffer.getByteBuffer().put(response.getByteBuffer());
//					response.setLimit(orgLimit);
					response.transferTo(inputBuffer);
...
//					inputBuffer.getByteBuffer().reset();
				...
				// Get some more data
				if (isOpen()) {
					channel.read(response.getByteBuffer(), null, handler);
				} else {
...
//			response.flip();
		...
		public void failed(Throwable exc, Void attachment) {
			if (exc instanceof ReadBufferOverflowException) {
				...
				response = ByteBufferWrapper
						.wrapper(ByteBuffer.allocate(((ReadBufferOverflowException) exc).getMinBufferSize()), false);
//				response.flip();
				response.switchToReadMode();
