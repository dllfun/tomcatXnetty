...
public class Socket {
	 ...
	 * APR_TCP_NODELAY set to tell us that APR_TCP_NODELAY should be turned on again
	 * when NOPUSH is turned off
	 */
	public static final int APR_RESET_NODELAY = 2048;
	/**
	 * Set on non-blocking sockets (timeout != 0) on which the previous read() did
	 * not fill a buffer completely. the next apr_socket_recv() will first call
	 * select()/poll() rather than going straight into read(). (Can also be set by
	 * an application to force a select()/poll() call before the next read, in cases
	 * where the app expects that an immediate read would fail.)
	 */
	 ...
	 * calls are still valid on the socket.
	 * 
	 ...
	 * @param sa   The socket address to bind to This may be where we will find out
	 *             if there is any other process using the selected port.
	 * @return the operation status
	 */
	 ...
	 * @return A copy of the socket that is connected to the socket that made the
	 *         connection request. This is the socket which should be used for all
	 ...
	 * @return <code>true</code> if socket is at the OOB/urgent mark, otherwise
	 *         <code>false</code>.
	 */
	 ...
	 * @param offset Offset in the byte buffer.
	 * @param nbytes The number of bytes to read (-1) for full array.
	 * @return If &gt; 0, the return value is the number of bytes read. If == 0, the
	 *         return value indicates EOF and if &lt; 0 the return value is the
	 *         error code. Note a non-blocking read with no data current available
	 ...
	 * behavior, use apr_socket_timeout_set() or the APR_SO_NONBLOCK socket option.
	 * The number of bytes actually sent is stored in the len parameter. The offset
	 * parameter is passed by reference for no reason; its value will never be
	 * modified by the apr_socket_sendfile() function.
	 * 
