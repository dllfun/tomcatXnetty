...
/**
 * <p>
 * Tomcat port of <a href=
 * "https://httpd.apache.org/docs/trunk/mod/mod_remoteip.html">mod_remoteip</a>,
 ...
 * <code>request.scheme = https</code> and
 * <code>request.serverPort = 443</code>. Note that 443 can be overwritten with
 * the <code>$httpsServerPort</code> configuration parameter.</li>
 * <li>Mark the request with the attribute
 * {@link Globals#REQUEST_FORWARDED_ATTRIBUTE} and value {@code Boolean.TRUE} to
 ...
 * 172\.3[0-1]{1}\.\d{1,3}\.\d{1,3}| 0:0:0:0:0:0:0:1|::1 <br>
 ...
 * when the <code>protocolHeader</code> indicates <code>http</code>
 * protocol</td>
 * <td>N/A</td>
 * <td>integer</td>
 * <td>80</td>
 ...
 * <td>Value returned by {@link javax.servlet.ServletRequest#getServerPort()}
 * when the <code>protocolHeader</code> indicates <code>https</code>
 * protocol</td>
 * <td>N/A</td>
 * <td>integer</td>
 * <td>443</td>
 ...
 * trusted and thus, we cannot trust that <code>untrusted-proxy</code> is the
...
public class RemoteIpValve extends ValveBase {
	 ...
	 * {@link Request#getServerName()} and (optionally depending on {link
	...
	public void invoke(Request request, Response response) throws IOException, ServletException {
		...
		if (isInternal || (trustedProxies != null && trustedProxies.matcher(originalRemoteAddr).matches())) {
			...
			// loop on remoteIpHeaderValue to find the first trusted remote ip and to build
			// the proxies chain
			...
			if (protocolHeader != null) {
				...
				if (protocolHeaderValue == null) {
					...
					request.getCoyoteRequest().getScheme().setString("https");
					setPorts(request, httpsServerPort);
				} else {
					request.setSecure(false);
					request.getCoyoteRequest().getScheme().setString("http");
			...
			if (hostHeader != null) {
				...
				if (hostHeaderValue != null) {
					try {
						...
						request.getCoyoteRequest().getServerName().setString(hostHeaderValue);
						if (isChangeLocalName()) {
							request.getCoyoteRequest().getLocalName().setString(hostHeaderValue);
						}
		...
		try {
			...
			request.getCoyoteRequest().getScheme().setString(originalScheme);
			request.getCoyoteRequest().getServerName().setString(originalServerName);
			request.getCoyoteRequest().getLocalName().setString(originalLocalName);
	...
	/*
	 * Considers the value to be secure if it exclusively holds forwards for {@link
	...
	/**
	 * <p>
	 * Server Port value if the {@link #protocolHeader} is not <code>null</code> and
	...
	/**
	 * <p>
	 * Case insensitive value of the protocol header to indicate that the incoming
	 * http request uses SSL.
	 * </p>
	 * <p>
	 ...
	 * collect a list of all of the intermediate client IP addresses trusted to
	 * resolve the actual remote IP. Note that intermediate RemoteIPTrustedProxy
	...
	/**
	 * Should this valve set request attributes for IP address, Hostname, protocol
	 * and port used for the request? This are typically used in conjunction with
	 * the {@link AccessLog} which will otherwise log the original values. Default
	 * is <code>true</code>.
	 *
	 ...
	 * </ul>
	 *
	 * @param requestAttributesEnabled <code>true</code> causes the attributes to be
	 *                                 set, <code>false</code> disables the setting
	 *                                 of the attributes.
	 */
