...
public class RewriteValve extends ValveBase {
	...
	/**
	 * Relative path to the configuration file. Note: If the valve's container is a
	 * context, this will be relative to /WEB-INF/.
	 */
	...
	protected synchronized void startInternal() throws LifecycleException {
		...
		if (getContainer() instanceof Context) {
			context = true;
			is = ((Context) getContainer()).getServletContext().getResourceAsStream("/WEB-INF/" + resourcePath);
		...
		while (true) {
			try {
				...
				if (result instanceof RewriteRule) {
					...
					if (containerLog.isDebugEnabled()) {
						containerLog.debug("Add rule with pattern " + rule.getPatternString() + " and substitution "
					...
					for (RewriteCond condition : conditions) {
						if (containerLog.isDebugEnabled()) {
							...
							containerLog.debug("Add condition " + cond.getCondPattern() + " test "
		...
		try {
			...
			for (int i = 0; i < rules.length; i++) {
				...
				if (rule.isRedirect() && newtest != null) {
					...
					StringBuffer urlStringEncoded = new StringBuffer(
							URLEncoder.DEFAULT.encode(urlStringDecoded, uriCharset));
			...
			if (rewritten) {
				if (!done) {
					...
					request.getCoyoteRequest().getRequestURI().setString(null);
					CharChunk chunk = request.getCoyoteRequest().getRequestURI().getCharChunk();
					chunk.recycle();
					if (context) {
					...
					request.getCoyoteRequest().getRequestURI().toChars();
					// Decoded and normalized URI
					// Rewriting may have denormalized the URL
					urlStringDecoded = RequestUtil.normalize(urlStringDecoded);
					request.getCoyoteRequest().getDecodedURI().setString(null);
					chunk = request.getCoyoteRequest().getDecodedURI().getCharChunk();
					chunk.recycle();
					if (context) {
					...
					request.getCoyoteRequest().getDecodedURI().toChars();
					// Set the new Query if there is one
					if (queryStringDecoded != null) {
						request.getCoyoteRequest().getQueryString().setString(null);
						chunk = request.getCoyoteRequest().getQueryString().getCharChunk();
						chunk.recycle();
						...
						if (!chunk.isNull()) {
							request.getCoyoteRequest().getQueryString().toChars();
						}
					}
					// Set the new host if it changed
					if (!host.equals(request.getServerName())) {
						request.getCoyoteRequest().getServerName().setString(null);
						chunk = request.getCoyoteRequest().getServerName().getCharChunk();
						chunk.recycle();
						chunk.append(host.toString());
						request.getCoyoteRequest().getServerName().toChars();
					}
	 ...
	 *
	 * Example: RewriteCond %{REMOTE_HOST} ^host1.* [OR]
	 *
	...
	public static Object parse(String line) {
		...
		if (tokenizer.hasMoreTokens()) {
			...
			if (token.equals("RewriteCond")) {
				...
				// FIXME: Possibly implement more special maps from
