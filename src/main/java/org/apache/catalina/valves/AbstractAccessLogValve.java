...
/**
 * <p>
 * Abstract implementation of the <b>Valve</b> interface that generates a web
 ...
 * <code>mod_log_config</code> module.
 * </p>
 *
 * <p>
 ...
 * <li><code>+</code> = Connection may be kept alive after the response is
 * sent.</li>
 ...
 * It is modeled after the <a href="https://httpd.apache.org/">Apache HTTP
...
public abstract class AbstractAccessLogValve extends ValveBase implements AccessLog {
	...
	/**
	 * Name of locale used to format timestamps in log entries and in log file name
	 * suffix.
	 */
	...
	/**
	 * Should this valve use request attributes for IP address, hostname, protocol
	 * and port used for the request. Default is <code>false</code>.
	 * 
	...
	/**
	 * Log message buffers are usually recycled and re-used. To prevent excessive
	 * memory usage, if a buffer grows beyond this size it will be discarded. The
	 * default is 256 characters. This should be set to larger than the typical
	...
	/**
	 * Return the locale used to format timestamps in log entries and in log file
	 * name suffix.
	 * 
	 * @return the locale
	 */
	...
	/**
	 * Set the locale used to format timestamps in log entries and in log file name
	 * suffix. Changing the locale is only supported as long as the AccessLogValve
	 * has not logged anything. Changing the locale later can lead to inconsistent
	 * formatting.
	 *
	...
	/**
	 * Log a message summarizing the specified request and response, according to
	 * the format specified by the <code>pattern</code> property.
	 *
	 ...
	 *
	 * @param message Message to be logged. This object will be recycled by the
	 *                calling method.
	 */
	...
	/**
	 * This method returns a Date object that is accurate to within one second. If a
	 * thread calls this method to get a Date and it's been less than 1 second since
	 * a new Date was created, this method simply gives out the same Date again so
	 * that the system doesn't spend time creating Date objects unnecessarily.
	 * 
	...
	/**
	 * Marks an AccessLogElement as needing to be have the value cached at the start
	 * of the request rather than just recorded at the end as the source data for
	 * the element may not be available at the end of the request. This typically
	 * occurs for remote network information, such as ports, IP addresses etc. when
	 * the connection is closed unexpectedly. These elements take advantage of these
	 * values being cached elsewhere on first request and do not cache the value in
	 * the element since the elements are state-less.
	 */
	...
	protected static class ElapsedTimeElement implements AccessLogElement {
		...
		/**
		 * @param millis <code>true</code>, write time in millis - %D, if
		 *               <code>false</code>, write time in seconds - %T
		 */
	...
	protected static class ConnectionStatusElement implements AccessLogElement {
		...
		public void addElement(CharArrayWriter buf, Date date, Request request, Response response, long time) {
			if (response != null && request != null) {
				...
				// If status is not found yet, cont to check whether connection is keep-alive or
				// close
				if (!statusFound) {
