...
public class AprLifecycleListener implements LifecycleListener {
	...
	/**
	 * Info messages during init() are cached until Lifecycle.BEFORE_INIT_EVENT so
	 * that, in normal (non-error) cases, init() related log messages appear at the
	 * expected point in the lifecycle.
	 */
	 ...
	 * <code>FIPS_mode_set()</code> to enable FIPS mode and that we expect as the
	 * return value of <code>FIPS_mode()</code> when FIPS mode is enabled.
	 * <p>
	 * In the future the OpenSSL library might grow support for different non-zero
	 * "FIPS" modes that specify different allowed subsets of ciphers or whatever,
	 * but nowadays only "1" is the supported value.
	 * </p>
	 * 
	 * @see <a href="http://wiki.openssl.org/index.php/FIPS_mode_set%28%29">OpenSSL
	...
	private static void init() {
		...
		if (apver < rqver) {
			log.error(sm.getString("aprListener.tcnInvalid", major + "." + minor + "." + patch,
		...
		if (apver < rcver) {
			initInfoLogMessages.add(sm.getString("aprListener.tcnVersion", major + "." + minor + "." + patch,
					TCN_REQUIRED_MAJOR + "." + TCN_RECOMMENDED_MINOR + "." + TCN_RECOMMENDED_PV));
		}

		initInfoLogMessages.add(sm.getString("aprListener.tcnValid", major + "." + minor + "." + patch,
				Library.APR_MAJOR_VERSION + "." + Library.APR_MINOR_VERSION + "." + Library.APR_PATCH_VERSION));
