...
public class AsyncContextImpl implements AsyncContext, AsyncContextCallback {
	 ...
	 * non-container threads it is possible for a previous dispatch() to interfere
	 * with a following start(). This lock prevents that from happening. It is a
	 * dedicated object as user code may lock on the AsyncContext so if container
	 * code also locks on that object deadlocks may occur.
	 */
	...
	public void dispatch(ServletContext servletContext, String path) {
		synchronized (asyncContextLock) {
			...
			Runnable dispatch = new AsyncRunnable(request, applicationDispatcher, servletRequest, servletResponse);
