...
public class CoyoteAdapter implements Adapter {
	...
	@Override
	public boolean asyncDispatch(org.apache.coyote.Request req, org.apache.coyote.Response res, SocketEvent event)
			throws Exception {
		...
		try {
			...
			if (!request.isAsyncDispatching() && request.isAsync()) {
				...
				if (writeListener != null && event == SocketEvent.OPEN_WRITE) {
					ClassLoader oldCL = null;
					try {
						oldCL = request.getContext().bind(false, null);
						res.onWritePossible();
				...
				} else if (readListener != null && event == SocketEvent.OPEN_READ) {
					ClassLoader oldCL = null;
					try {
						oldCL = request.getContext().bind(false, null);
			...
			if (request.isAsyncDispatching()) {
				Runnable dispatch = request.pushDispatchingState();
				try {
					dispatch.run();
				} catch (Throwable t) {
					ExceptionUtils.handleThrowable(t);
	...
	/**
	 * Perform the necessary processing after the HTTP headers have been parsed to
	 * enable the request/response pair to be passed to the start of the container
	 ...
	 *
	 * @return <code>true</code> if the request should be passed on to the start of
	 *         the container pipeline, otherwise <code>false</code>
	 *
	...
	protected boolean postParseRequest(org.apache.coyote.Request req, Request request, org.apache.coyote.Response res,
		...
		if (req.getScheme().isNull()) {
			// Use connector scheme and secure configuration, (defaults to
			// "http" and false respectively)
			req.getScheme().setString(connector.getScheme());
			request.setSecure(connector.getSecure());
		} else {
			// Use processor specified scheme to determine secure state
			request.setSecure(req.getScheme().equals("https"));
		}
		...
		if (proxyName != null) {
			req.getServerName().setString(proxyName);
		}

		MessageBytes undecodedURI = req.getRequestURI();
		...
		MessageBytes decodedURI = req.getDecodedURI();
		...
		if (undecodedURI.getType() == MessageBytes.T_BYTES) {
			...
			// Normalization
			if (normalize(req.getDecodedURI())) {
				// Character decoding
				...
				// removed
				if (!checkNormalize(req.getDecodedURI())) {
					response.sendError(400, "Invalid URI");
				}
			} else {
				response.sendError(400, "Invalid URI");
			 ...
			 * handler. The following assumptions are made: - req.requestURI() has been set
			 * to the 'original' non-decoded, non-normalized URI - req.decodedURI() has been
			 * set to the decoded, normalized form of req.requestURI()
			 */
			decodedURI.toChars();
		...
		if (connector.getUseIPVHosts()) {
			serverName = req.getLocalName();
			if (serverName.isNull()) {
				// well, they did ask for it
				req.actionREQ_LOCAL_NAME_ATTRIBUTE();
			}
		} else {
		...
		// Filter trace method
		if (!connector.getAllowTrace() && req.getMethod().equalsIgnoreCase("TRACE")) {
			Wrapper wrapper = request.getWrapper();
			String header = null;
			if (wrapper != null) {
	...
	/**
	 * Extract the path parameters from the request. This assumes parameters are of
	 * the form /path;name=value;name2=value2/ etc. Currently only really interested
	 * in the session ID that will be in this form. Other parameters can safely be
	 * ignored.
	 *
	 * @param req     The Coyote request object
	...
	protected void parsePathParameters(org.apache.coyote.Request req, Request request) {
		...
		req.getDecodedURI().toBytes();

		ByteChunk uriBC = req.getDecodedURI().getByteChunk();
	...
	/**
	 * Look for SSL session ID if required. Only look for SSL Session ID if it is
	 * the only tracking method enabled.
	 *
	 * @param request The Servlet request object
	 */
	 ...
	 *
	 * @param mb The MessageBytes instance containing the bytes that should be
	 *           converted to chars
	 */
	 ...
	 *
	 * @return <code>false</code> if normalizing this URI would require going above
	 *         the root, or if the URI contains a null byte, otherwise
	 *         <code>true</code>
	 */
	...
	/**
	 * Check that the URI is normalized following character decoding. This method
	 * checks for "\", 0, "//", "/./" and "/../".
	 *
	 * @param uriMB URI to be checked (should be chars)
	 *
	 * @return <code>false</code> if sequences that are supposed to be normalized
	 *         are still present in the URI, otherwise <code>true</code>
	 *
