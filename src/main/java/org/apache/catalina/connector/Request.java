...
public class Request implements HttpServletRequest {
	 ...
	 *
	 * @param connector The Connector with which this Request object will always be
	 *                  associated. In normal usage this must be non-null. In some
	 *                  test scenarios, it may be possible to use a null Connector
	...
	/**
	 * Flag that indicates if SSL attributes have been parsed to improve performance
	 * for applications (usually frameworks) that make multiple calls to
	 * {@link Request#getAttributeNames()}.
	 */
	...
	/**
	 * Cookie parsed flag. Indicates that the ServerCookies have been converted into
	 * user facing Cookie objects.
	 */
...
//	private volatile Runnable dispatch = null;
	 ...
	 * <p>
	 * This is available as soon as the appropriate Context is identified. Note that
	 * availability of a Context allows <code>getContextPath()</code> to return a
	 * value, and thus enables parsing of the request URI.
	 *
	 ...
	 * 
	 * @return the value of the flag as set on the connector, or <code>true</code>
	 ...
	 * {@link #getRequest()}, wrap it and then call this method with the wrapped
	 * request.
	 *
	...
	public Object getAttribute(String name) {
		...
		if (TLSUtil.isTLSRequestAttribute(name)) {
			coyoteRequest.actionREQ_SSL_ATTRIBUTE();
	 ...
	 * {@link #setAttribute(String, Object)}. Tomcat internal attributes will not be
	 * included although they are accessible via {@link #getAttribute(String)}. The
	 * Tomcat internal attributes include:
	 * <ul>
	 ...
	 * </ul>
	 * The underlying connector may also expose request attributes. These all have
	 * names starting with "org.apache.tomcat" and include:
	 * <ul>
	 * <li>{@link Globals#SENDFILE_SUPPORTED_ATTR}</li>
	 * </ul>
	 * Connector implementations may return some, all or none of these attributes
	 * and may also support additional attributes.
	 *
	 ...
	 *         returns a servlet input stream created by
	 ...
	 *         based on the values for any <code>Accept-Language</code> headers that
	 *         were encountered. If the request did not specify a preferred
	...
	/**
	 * @return the Internet Protocol (IP) source port of the client or last proxy
	 *         that sent the request.
	 */
	@Override
	public int getRemotePort() {
		if (remotePort == -1) {
			coyoteRequest.actionREQ_REMOTEPORT_ATTRIBUTE();
	...
	/**
	 * @return the host name of the Internet Protocol (IP) interface on which the
	...
	public int getLocalPort() {
		if (localPort == -1) {
			coyoteRequest.actionREQ_LOCALPORT_ATTRIBUTE();
	...
	public RequestDispatcher getRequestDispatcher(String path) {
		 ...
		 *
		 * From the Servlet 4.0 Javadoc: - The pathname specified may be relative,
		 * although it cannot extend outside the current servlet context. - If it is
		 * relative, it must be relative against the current servlet
		 *
		 * From Section 9.1 of the spec: - The servlet container uses information in the
		 * request object to transform the given relative path against the current
		 * servlet to a complete path.
		 *
		 * It is undefined whether the requestURI is used or whether servletPath and
		 * pathInfo are used. Given that the RequestURI includes the contextPath (and
		 * extracting that is messy) , using the servletPath and pathInfo looks to be
...
//	}
	...
	/**
	 * Set a flag indicating whether or not the requested session ID for this
	 * request came in through SSL. This is normally called by the HTTP Connector,
	 * when it parses the request headers.
	 *
	 * @param flag The new flag
	 */
	...
	public <T extends HttpUpgradeHandler> T upgrade(Class<T> httpUpgradeHandlerClass)
		...
		try {
			// Do not go through the instance manager for internal Tomcat classes since they
			// don't
			// need injection
	...
	private int nextSlash(char[] uri, int startPos) {
		...
		while (pos < len) {
			if (uri[pos] == '/') {
				return pos;
			} else if (connector.getEncodedSolidusHandlingInternal() == EncodedSolidusHandling.DECODE && uri[pos] == '%'
					&& pos + 2 < len && uri[pos + 1] == '2' && (uri[pos + 2] == 'f' || uri[pos + 2] == 'F')) {
				return pos;
			}
			pos++;
		}
	...
	/**
	 * Return the set of Cookies received with this Request. Triggers parsing of the
	 * Cookie HTTP headers followed by conversion to Cookie objects if this has not
	 ...
	 * Triggers parsing of the Cookie HTTP headers (but not conversion to Cookie
	 * objects) if the headers have not yet been parsed.
	 *
	...
	/**
	 * Change the ID of the session that this request is associated with. There are
	 * several things that may trigger an ID change. These include moving between
	 * nodes in a cluster and session fixation prevention during the authentication
	 * process.
	 *
	 ...
	 * disable the swallowing of any remaining input and close the connection once
	...
	protected Session doGetSession(boolean create) {
		...
		if (requestedSessionSSL) {
			...
			/*
			 * This is the common(ish) use case: using the same session ID with multiple web
			 * applications on the same host. Typically this is used by Portlet
			 * implementations. It only works if sessions are tracked via cookies. The
			 * cookie must have a path of "/" else it won't be provided for requests to all
			 * web applications.
			 *
			 * Any session ID provided by the client should be for a session that already
			 * exists somewhere on the host. Check if the context is configured for this to
			 * be confirmed.
			 */
	...
	/**
	 * Converts the parsed cookies (parsing the Cookie headers first if they have
	 * not been parsed) into Cookie objects.
	 */
	...
	static {
		...
		specialAttributes.put(Globals.CONNECTION_ID, new SpecialAttributeAdapter() {
			...
			public Object get(Request request, String name) {
				...
				request.getCoyoteRequest().actionCONNECTION_ID(result);
				return result.get();
			}
		...
		specialAttributes.put(Globals.STREAM_ID, new SpecialAttributeAdapter() {
			...
			public Object get(Request request, String name) {
				AtomicReference<Object> result = new AtomicReference<>();
				request.getCoyoteRequest().actionSTREAM_ID(result);
				return result.get();
			}
