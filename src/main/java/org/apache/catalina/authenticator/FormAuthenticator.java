...
public class FormAuthenticator extends AuthenticatorBase {
	...
	/**
	 * Character encoding to use to read the username and password parameters from
	 * the request. If not set, the encoding of the request body will be used.
	 */
	protected String characterEncoding = null;

	/**
	 * Landing page to use if a user tries to access the login page directly or if
	 * the session times out during login. If not set, error responses will be sent
	 * instead.
	 */
	 ...
	 * configuration. Return <code>true</code> if any specified constraint has been
	...
	protected void register(Request request, HttpServletResponse response, Principal principal, String authType,
		...
		// store username and password as session notes to use them for
		// re-authentication.
		if (!cache) {
			Session session = request.getSessionInternal(false);
			if (session != null) {
	 ...
	 *                     {@link HttpServletResponse#sendError(int, String)} throws
	 *                     an {@link IOException}
	 */
	protected void forwardToLoginPage(Request request, HttpServletResponse response, LoginConfig config)
		...
		request.getCoyoteRequest().getMethod().setString("GET");
		...
		try {
			...
			request.getCoyoteRequest().getMethod().setString(oldMethod);
		}
	}
	 ...
	 * original request is no longer present (because the session timed out), return
	...
	protected boolean restoreRequest(Request request, Session session) throws IOException {
		...
		if (body != null) {
			request.getCoyoteRequest().reqSetBodyReplay(body);
		...
		request.getCoyoteRequest().getMethod().setString(method);
	...
	/**
	 * Return the request URI (with the corresponding query string, if any) from the
	 * saved request so that we can redirect to it.
	 *
