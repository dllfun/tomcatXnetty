 ...
 *
 * It is possible for a stream to be waiting for a connection allocation when a
 * stream allocation is made. Therefore this class tracks the type of allocation
 * that the stream is waiting for to ensure that notifications are correctly
 * triggered.
 *
...
class WindowAllocationManager {
	...
	void waitForStream(long timeout) throws InterruptedException {
		if (log.isDebugEnabled()) {
			log.debug(sm.getString("windowAllocationManager.waitFor.stream", stream.getConnectionId(),
					stream.getIdentifier(), Long.toString(timeout)));
		}
	...
	private void notify(int notifyTarget) {
		if (log.isDebugEnabled()) {
			log.debug(sm.getString("windowAllocationManager.notify", stream.getConnectionId(), stream.getIdentifier(),
					Integer.toString(waitingFor), Integer.toString(notifyTarget)));
		}
		...
		synchronized (stream) {
			if ((notifyTarget & waitingFor) > NONE) {
				...
				// unexpected timeouts.
				waitingFor = NONE;
				if (stream.getProcessor().getAsyncStateMachine().getWriteListener() == null) {
