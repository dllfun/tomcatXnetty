...
public class HPackHuffman {
	 ...
	 * <p/>
	 * the high two bytes corresponds to the tree node if the bit is set, and the
	 * low two bytes for if it is clear if the high bit is set it is a terminal
	 * node, otherwise it contains the next node position.
	 */
	...
	static {
		...
		// lengths determined by experimentation, just set it to something large then
		// see how large it actually ends up
	...
	/**
	 * Decodes a huffman encoded string into the target StringBuilder. There must be
	 * enough space left in the buffer for this method to succeed.
	 *
	 * @param data   The byte buffer
	 * @param length The length of data from the buffer to decode
	 * @param target The target for the decompressed data
	 *
	 * @throws HpackException If the Huffman encoded value in HPACK headers did not
	 *                        end with EOS padding
	 */
	...
	/**
	 * Encodes the given string into the buffer. If there is not enough space in the
	 * buffer, or the encoded version is bigger than the original it will return
	 * false and not modify the buffers position.
	 *
	...
	public static boolean encode(ByteBuffer buffer, String toEncode, boolean forceLowercase) {
		...
		// and we don't really have any option but to calculate it in advance to make
		// sure we have left enough room
