...
import org.apache.tomcat.util.net.AbstractChannel;
import org.apache.tomcat.util.res.StringManager;

/**
 * Used to managed prioritisation.
 */
abstract class AbstractStream extends AbstractChannel {
	...
	AbstractStream(Integer identifier) {
		System.out.println("stream(" + identifier + ") created");
	 ...
	 * @throws Http2Exception If the window size is now higher than the maximum
	 *                        allowed
	 */
	synchronized void incrementWindowSize(int increment) throws Http2Exception {
		...
		if (log.isDebugEnabled()) {
			log.debug(sm.getString("abstractStream.windowSizeInc", getConnectionId(), getIdentifier(),
