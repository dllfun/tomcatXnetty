...
public abstract class AbstractHttp11Protocol<S> extends AbstractProtocol<S> {
	...
	public void init() throws Exception {
		...
		// upgradeProtocols.add(new Http2Protocol());
	 ...
	 */
	// @Override
	// protected Endpoint<S> getEndpoint() {
	...
	/**
	 * If an HTTP request is received that contains an illegal header name or value
	 * (e.g. the header name is not a token) will the request be rejected (with a
	 * 400 response) or will the illegal header be ignored?
	 *
	 ...
	 *                                ignore the header
	 *
	 * @deprecated Now an alias for {@link #setRejectIllegalHeader(boolean)}. Will
	 ...
	 * constraints require a client certificate, the POST body needs to be buffered
	 * while an SSL handshake takes place to obtain the certificate. A similar
	 * buffering is required during FDORM auth.
	 *
	 ...
	 * Default is 5 minutes as in Apache HTTPD server.
	 *
	 ...
	 * connectionUploadTimeout will be ignored and the regular socket timeout will
	 * be used for the full duration of the connection.
	 *
	...
	/**
	 * Get the string form of the regular expression that defines the User agents
	 * which should be restricted to HTTP/1.0 support.
	 *
	...
	protected Processor createProcessor() {
		Http11Processor processor = new Http11Processor(this, getAdapter());
		return processor;
	}
