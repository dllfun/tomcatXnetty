...
public class SavedRequestInputFilter implements InputFilter {
	 ...
	 * (input.getOffset() >= input.getEnd()) return -1;
	 * 
	 * BufWrapper byteBuffer = handler.getBufWrapper();
	 * byteBuffer.setPosition(byteBuffer.getLimit());
	 * byteBuffer.setLimit(byteBuffer.getCapacity());
	 * input.subtract(byteBuffer.nioBuffer());
	 * 
