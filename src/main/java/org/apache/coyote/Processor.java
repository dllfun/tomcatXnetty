...
public interface Processor {
	 ...
	 *
	 * @return An upgrade token encapsulating the information required to process
	 *         the upgrade request
	 *
	 * @throws IllegalStateException if this is called on a Processor that does not
	 *                               support upgrading
	 */
	 ...
	 *
	 * @param now The time (as returned by {@link System#currentTimeMillis()} to use
	 *            as the current time to determine whether the timeout has expired.
	...
	/**
	 * Informs the processor that the underlying I/O layer has stopped accepting new
	 * connections. This is primarily intended to enable processors that use
	 * multiplexed connections to prevent further 'streams' being added to an
	 ...
	 * generation of the AsyncStateMachine) is the same as the generation when the
	 * most recent async timeout was triggered. This is intended to be used to avoid
