...
//			}
	 ...
	 * requests and for requests that have partially read the HTTP request line or
	 * HTTP headers. Once the headers have been fully read this method is not called
	 * again until there is a new HTTP request to process. Note that the request
	 * type may change during processing which may result in one or more calls to
	 * {@link #dispatch(SocketEvent)}. Requests may be pipe-lined.
	 *
	 * @param socketWrapper The connection to process
	 *
	 ...
	 * currently include Servlet 3.0 Async and HTTP upgrade connections. Further
	 * uses may be added in the future. These will typically start as HTTP requests.
	 *
