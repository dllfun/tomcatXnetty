...
//	}
	...
	public class HeadHandler<S> implements Handler<S> {

		private Handler<S> next;

		public HeadHandler(Handler<S> next) {
			super();
			this.next = next;
		}
		...
		@Override
		public AbstractProtocol<S> getProtocol() {
			return (AbstractProtocol<S>) AbstractProtocol.this;
		}
	...
	public class TailHandler<S> implements Handler<S> {
		...
		private SocketState process(SocketChannel channel, SocketEvent event) {
			...
			try {
				if (processor == null) {
					...
					// returns "" when no protocol is negotiated
					if (negotiatedProtocol != null && negotiatedProtocol.length() > 0) {
						UpgradeProtocol upgradeProtocol = getNegotiatedProtocol(negotiatedProtocol);
						if (upgradeProtocol != null) {
							processor = upgradeProtocol.getProcessor(channel, getAdapter());
							if (getLog().isDebugEnabled()) {
								getLog().debug(sm.getString("abstractConnectionHandler.processorCreate", processor));
							}
				...
				do {
					...
					if (state == SocketState.UPGRADING) {
						...
						if (upgradeToken == null) {
							...
							UpgradeProtocol upgradeProtocol = getUpgradeProtocol("h2c");
							if (upgradeProtocol != null) {
								processor = upgradeProtocol.getProcessor(channel, getAdapter());
							...
							processor = createUpgradeProcessor(channel, upgradeToken);
							if (getLog().isDebugEnabled()) {
								getLog().debug(
										sm.getString("abstractConnectionHandler.upgradeCreate", processor, channel));
							}
							channel.unRead(leftOverInput);
