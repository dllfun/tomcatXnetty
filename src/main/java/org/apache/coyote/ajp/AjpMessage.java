...
public class AjpMessage {
	...
	/**
	 * This actually means different things depending on whether the packet is read
	 * or write. For read, it's the length of the payload (excluding the header).
	 * For write, it's the length of the packet as a whole (counting the header).
	 * Oh, well.
	 */
	protected int len;
	...
	/**
	 * Prepare this packet for accumulating a message from the container to the web
	 * server. Set the write position to just after the header (but leave the length
	 * unwritten, because it is as yet unknown).
	 */
	public void reset() {
		len = 4;
		pos = 4;
	...
	/**
	 * For a packet to be sent to the web server, finish the process of accumulating
	 * data and write the length of the data payload into the header.
	 */
	public void end() {
		len = pos;
		int dLen = len - 4;
	 ...
	 *
	 * @return For read, it's the length of the payload (excluding the header). For
	 *         write, it's the length of the packet as a whole (counting the
	 *         header).
	 */
	public int getLen() {
		return len;
	...
	/**
	 * Write a MessageBytes out at the current write position. A null MessageBytes
	 ...
	 * position. The chunk of bytes is encoded with the length in two bytes first,
	 * then the data itself, and finally a terminating \0 (which is <B>not</B>
	 * included in the encoded length).
	 *
	 ...
	 * @param b The ByteBuffer from which to copy bytes.
	 */
	public void appendBytes(BufWrapper b) {
		int numBytes = b.getRemaining();
		if (checkOverflow(numBytes)) {
			return;
		}
		appendInt(numBytes);
	...
	/**
	 * Read an integer from packet, and advance the read position past it. Integers
	 * are encoded as two unsigned bytes with the high-order byte first, and, as far
	 * as I can tell, in little-endian order within each byte.
	 *
	 ...
	 * Integers are encoded as four unsigned bytes with the high-order byte first,
	 * and, as far as I can tell, in little-endian order within each byte.
	 *
