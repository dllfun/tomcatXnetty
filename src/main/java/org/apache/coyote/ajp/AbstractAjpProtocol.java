 ...
 * typically extend this base class rather than implement
 * {@link org.apache.coyote.ProtocolHandler}. All of the implementations that
 * ship with Tomcat are implemented this way.
 *
...
public abstract class AbstractAjpProtocol<S> extends AbstractProtocol<S> {
	...
	public AbstractAjpProtocol(Endpoint<S> endpoint) {
		super(endpoint);
	...
	/**
	 * Configure whether to aend an AJP flush packet when flushing. A flush packet
	 * is a zero byte AJP13 SEND_BODY_CHUNK packet. mod_jk and mod_proxy_ajp
	 * interpret this as a request to flush data to the client. AJP always does
	 * flush at the and of the response, so if it is not important, that the packets
	 * get streamed up to the client, do not use extra flush packets. For
	 * compatibility and to stay on the safe side, flush packets are enabled by
	 * default.
	 *
	 ...
	 *
	 * @deprecated Replaced by {@link #setSecret(String)}. Will be removed in Tomcat
	 *             11 onwards
	 */
	@Deprecated
	 ...
	 *
	 * @deprecated Replaced by {@link #getSecret()}. Will be removed in Tomcat 11
	 *             onwards
	 */
	@Deprecated
